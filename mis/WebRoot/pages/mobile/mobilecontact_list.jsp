...
<script type="text/javascript">
	...
	function tableDataInit() {
		$("#tableData").datagrid({
			...
			fit : true,
			url : "${ctx}/mobileContact.do?method=list",
	...
	function add() {
		...
		window.parent.$("#modalWindow").html("<div id=\"mobileContactModalWindow\" modal=\"true\" shadow=\"false\" minimizable=\"false\" cache=\"false\" maximizable=\"false\" collapsible=\"false\" resizable=\"false\" style=\"margin: 0px;padding: 0px;overflow: auto;\"></div>");
		
		/* 初始化弹出页面的层 */
		window.parent.$("#mobileContactModalWindow").window({
			title : '新增',
			href : "${ctx}/pages/mobile/mobilecontact_addorupdate.jsp?op=" + operate,
			width : 520,
			height : 310,
			onClose: function () {
				window.parent.$("#mobileContactModalWindow").window('destroy');
			},
			onLoad : function() {
	...
	function edit(rowIndex) {
		...
		window.parent.$("#mobileContactModalWindow").window({
			...
			onLoad : function() {
				$.ajax({
					type : "post",
					url : "${ctx}/mobileContact.do?method=get",
					data : {
						"id" : rows.id
					},
					dataType : "json",
					...
					success : function(data) {
						window.parent.$('#tableForm').form('load', {
							...
							displayName3:data.displayName3,
							displayName4:data.displayName4,
							shortNum1:data.shortNum1,
							shortNum2:data.shortNum2,
							shortNum3:data.shortNum3,
							shortNum4:data.shortNum4,
							mobilePhone1:data.mobilePhone1,
							mobilePhone2:data.mobilePhone2,
							mobilePhone3:data.mobilePhone3,
							mobilePhone4:data.mobilePhone4,
							status:data.status
						});
					},
	...
	function addOrUpdate() {
		var url = "";
		if (operate == "add") {
			url = "${ctx}/mobileContact.do?method=add";
		} else if (operate == "update") {
			url = "${ctx}/mobileContact.do?method=update";
		}
	...
	function remove(){
		...
		window.parent.$.messager.confirm("确认", "是否确认删除", function(r) {
			if(r){
				$.post("${ctx}/mobileContact.do?method=delete", {ids:ids}, function(data) {
					if (data.message == "200") {
						actionSuccess();
						$('#tableData').datagrid("reload");
						rows.length=0;
	...
	function exprot() {
		...
		$.ajax({  
	        type:"POST",
	        dataType: "text",
	        url:"${ctx}/mobileContact.do?method=getfilepath",
