...
<script type="text/javascript">
	...
	function tableDataInit() {
		$("#tableData").datagrid({
			...
			fit : true,
			url : "${ctx}/mobileMenuConfig.do?method=list",
	...
	function add() {
		...
		window.parent.$("#modalWindow").html("<div id=\"mobileMenuConfigModalWindow\" modal=\"true\" shadow=\"false\" minimizable=\"false\" cache=\"false\" maximizable=\"false\" collapsible=\"false\" resizable=\"false\" style=\"margin: 0px;padding: 0px;overflow: auto;\"></div>");
		
		/* 初始化弹出页面的层 */
		window.parent.$("#mobileMenuConfigModalWindow").window({
			title : '新增',
			href : "${ctx}/pages/mobile/mobilemenuconfig_addorupdate.jsp?op=" + operate,
			width : 300,
			height : 290,
			onClose: function () {
				window.parent.$("#mobileMenuConfigModalWindow").window('destroy');
			},
			onLoad : function() {
	...
	function edit(rowIndex) {
		...
		window.parent.$("#mobileMenuConfigModalWindow").window({
			...
			onLoad : function() {
				$.ajax({
					type : "post",
					url : "${ctx}/mobileMenuConfig.do?method=get",
					data : {
						"id" : rows.id
					},
					dataType : "json",
	...
	function addOrUpdate() {
		var url = "";
		if (operate == "add") {
			url = "${ctx}/mobileMenuConfig.do?method=add";
		} else if (operate == "update") {
			url = "${ctx}/mobileMenuConfig.do?method=update";
		}
	...
	function remove(){
		...
		window.parent.$.messager.confirm("确认", "是否确认删除", function(r) {
			if(r){
				$.post("${ctx}/mobileMenuConfig.do?method=delete", {ids:ids}, function(data) {
					if (data.message == "200") {
						actionSuccess();
						$('#tableData').datagrid("reload");
						rows.length=0;
	...
	function exprot() {
		...
		$.ajax({  
	        type:"POST",
	        dataType: "text",
	        url:"${ctx}/mobileMenuConfig.do?method=getfilepath",
