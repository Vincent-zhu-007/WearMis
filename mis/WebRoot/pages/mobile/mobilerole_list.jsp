...
<script type="text/javascript">
	...
	function tableDataInit() {
		$("#tableData").datagrid({
			...
			fit : true,
			url : "${ctx}/mobileRole.do?method=list",
	...
	function add() {
		...
		window.parent.$("#modalWindow").html("<div id=\"mobileRoleModalWindow\" modal=\"true\" shadow=\"false\" minimizable=\"false\" cache=\"false\" maximizable=\"false\" collapsible=\"false\" resizable=\"false\" style=\"margin: 0px;padding: 0px;overflow: auto;\"></div>");
		
		/* 初始化弹出页面的层 */
		window.parent.$("#mobileRoleModalWindow").window({
			title : '新增',
			href : "${ctx}/pages/mobile/mobilerole_addorupdate.jsp?op=" + operate,
			width : 515,
			height : 500,
			onClose: function () {
				window.parent.$("#mobileRoleModalWindow").window('destroy');
			},
			onLoad : function() {
				//初始化树
	        	window.parent.$("#mobileRoleMenu").tree({
	        		url: "${ctx}/mobileRole.do?method=initmobilemenu",
	        		checkbox: true,
	...
	function edit(rowIndex) {
		...
		window.parent.$("#mobileRoleModalWindow").window({
			...
			onLoad : function() {
				$.ajax({
					type : "post",
					url : "${ctx}/mobileRole.do?method=get",
					data : {
						"id" : rows.id
					},
					dataType : "json",
					...
					success : function(data) {
						...
						window.parent.$("#mobileRoleMenu").tree({
			        		url: "${ctx}/mobileRole.do?method=initmobilemenubypermission&roleId=" + rows.id,
			        		checkbox: true,
			        		animate:false,/* 是否有动画效果 */
			        		onClick:function(node){
			        			window.parent.$(this).tree('toggle', node.target);
	...
	function addOrUpdate() {
		var url = "";
		if (operate == "add") {
			url = "${ctx}/mobileRole.do?method=add";
		} else if (operate == "update") {
			url = "${ctx}/mobileRole.do?method=update";
		}
	...
	function parentClearMenuConfigCache(){
		$.ajax({
			type:"POST",
	        url:"${ctx}/mobileMenuConfig.do?method=clearmobilemenuconfigcache",
	        async: false,
	        success: function(data){
	        	if(data.message == "200"){
	        		window.parent.$("#mobileRoleMenu").tree('reload');
	        		actionSuccess();
	        	}else{
	        		actionFailure();
	        	}
	...
	function remove(){
		...
		window.parent.$.messager.confirm("确认", "是否确认删除", function(r) {
			if(r){
				$.post("${ctx}/mobileRole.do?method=delete", {ids:ids}, function(data) {
					if (data.message == "200") {
						actionSuccess();
						$('#tableData').datagrid("reload");
						rows.length=0;
	...
	function exprot() {
		...
		$.ajax({  
	        type:"POST",
	        dataType: "text",
	        url:"${ctx}/mobileRole.do?method=getfilepath",
