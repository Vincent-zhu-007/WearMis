...
<script type="text/javascript">
	$(function(){
	    ...
	    $("#hasChildren").combobox({
		    url:"${ctx}/code.do?method=getcodecache&category=MOBILEMENUCONFIGHASCHILDREN",
		    valueField:'id',
		    textField:'text',
			required:true,
			editable:false
	    ...
	    $("#levelCode").combobox({
		    url:"${ctx}/code.do?method=getcodecache&category=MOBILEMENUCONFIGLEVELCODE",
		    valueField:'id',
		    textField:'text',
			required:true,
			editable:false,
	...
	function initParentCode(record){
		...
		if(levelCode != null && levelCode != ""){
			$("#parentCode").combobox({
			    url:"${ctx}/mobileMenuConfig.do?method=initparentcode&levelCode=" + levelCode,
			    valueField:'id',
			    textField:'text',
				required:true,
				editable:false
				...
				<li>
					...
					<span class="inputlable">
						<input id="code" name="code" class="easyui-validatebox" data-options="required:true,validType:{
							length:[0,50],
							remote:['${ctx}/mobileMenuConfig.do?method=isExist','code']
...
<script type="text/javascript">
	...
	function cancel() {
		$("#mobileMenuConfigModalWindow").window("destroy");
	}
</script>
