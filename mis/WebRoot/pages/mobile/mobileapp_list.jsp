...
<script type="text/javascript">
	...
	function tableDataInit() {
		$("#tableData").datagrid({
			...
			fit : true,
			url : "${ctx}/mobileApp.do?method=list",
	...
	function add() {
		...
		window.parent.$("#modalWindow").html("<div id=\"mobileAppModalWindow\" modal=\"true\" shadow=\"false\" minimizable=\"false\" cache=\"false\" maximizable=\"false\" collapsible=\"false\" resizable=\"false\" style=\"margin: 0px;padding: 0px;overflow: auto;\"></div>");
		
		/* 初始化弹出页面的层 */
		window.parent.$("#mobileAppModalWindow").window({
			title : '新增',
			href : "${ctx}/pages/mobile/mobileapp_addorupdate.jsp?op=" + operate,
			width : 520,
			height : 400,
			onClose: function () {
				window.parent.$("#mobileAppModalWindow").window('destroy');
			},
			onLoad : function() {
	...
	function edit(rowIndex) {
		...
		window.parent.$("#mobileAppModalWindow").window({
			...
			onLoad : function() {
				$.ajax({
					type : "post",
					url : "${ctx}/mobileApp.do?method=get",
					data : {
						"id" : rows.id
					},
					dataType : "json",
	...
	function addOrUpdate() {
		var url = "";
		if (operate == "add") {
			url = "${ctx}/mobileApp.do?method=add";
		} else if (operate == "update") {
			url = "${ctx}/mobileApp.do?method=update";
		}
	...
	function initListboxRemove(){
		$.ajax({
			url:"${ctx}/mobileAppConfig.do?method=getmobileappconfightmloptions",
			type:"POST",
			async:false,
			dataType:"text",
			success:function(data){
	...
	function initListboxRemoveAndListboxAdd(id){
		$.ajax({
			url:"${ctx}/mobileAppConfig.do?method=getremoveandaddmobileappconfightmloptions&id=" + id,
			type:"POST",
			async:false,
			dataType:"text",
			success:function(data){
	...
	function remove(){
		...
		window.parent.$.messager.confirm("确认", "是否确认删除", function(r) {
			if(r){
				$.post("${ctx}/mobileApp.do?method=delete", {ids:ids}, function(data) {
					if (data.message == "200") {
						actionSuccess();
						$('#tableData').datagrid("reload");
						rows.length=0;
	...
	function exprot() {
		...
		$.ajax({  
	        type:"POST",
	        dataType: "text",
	        url:"${ctx}/mobileApp.do?method=getfilepath",
