...
<script type="text/javascript">
	...
	function tableDataInit() {
		$("#tableData").datagrid({
			...
			fit : true,
			url : "${ctx}/webSiteMenuConfig.do?method=list",
			...
			{
				id : "clearWebSiteMenuConfigCacheBtn",
				text : '重置平台权限缓存',
				iconCls : 'icon-redo',
				handler : function() {
					clearWebSiteMenuConfigCache();
				}
			},'-',
			{
				id : "exportBtn",
	...
	function add() {
		...
		window.parent.$("#modalWindow").html("<div id=\"webSiteMenuConfigModalWindow\" modal=\"true\" shadow=\"false\" minimizable=\"false\" cache=\"false\" maximizable=\"false\" collapsible=\"false\" resizable=\"false\" style=\"margin: 0px;padding: 0px;overflow: auto;\"></div>");
		
		/* 初始化弹出页面的层 */
		window.parent.$("#webSiteMenuConfigModalWindow").window({
			title : '新增',
			href : "${ctx}/pages/mobile/websitemenuconfig_addorupdate.jsp?op=" + operate,
			width : 300,
			height : 310,
			onClose: function () {
				window.parent.$("#webSiteMenuConfigModalWindow").window('destroy');
			},
			onLoad : function() {
	...
	function edit(rowIndex) {
		...
		window.parent.$("#webSiteMenuConfigModalWindow").window({
			...
			onLoad : function() {
				$.ajax({
					type : "post",
					url : "${ctx}/webSiteMenuConfig.do?method=get",
					data : {
						"id" : rows.id
					},
					dataType : "json",
	...
	function addOrUpdate() {
		var url = "";
		if (operate == "add") {
			url = "${ctx}/webSiteMenuConfig.do?method=add";
		} else if (operate == "update") {
			url = "${ctx}/webSiteMenuConfig.do?method=update";
		}
	...
	function clearWebSiteMenuConfigCache(){
		$.ajax({
			type:"POST",
	...
	function remove(){
		...
		window.parent.$.messager.confirm("确认", "是否确认删除", function(r) {
			if(r){
				$.post("${ctx}/webSiteMenuConfig.do?method=delete", {ids:ids}, function(data) {
					if (data.message == "200") {
						actionSuccess();
						$('#tableData').datagrid("reload");
						rows.length=0;
	...
	function exprot() {
		...
		$.ajax({  
	        type:"POST",
	        dataType: "text",
	        url:"${ctx}/webSiteMenuConfig.do?method=getfilepath",
