...
<div class="easyui-layout" data-options="fit:true,border:false">
	<div data-options="region:'north',border:false" style="height: 40px; overflow: hidden;">
		<form id="searchForm">
			<ul>
				<li>设备号：<input id="equipmentNo" name="equipmentNo" /></li>
...
<script type="text/javascript">
	...
	function tableDataInit() {
		$("#tableData").datagrid({
			...
			fit : true,
			url : "${ctx}/mobileCardReader.do?method=list",
			...
			{
				id : "clearMobileCardReaderCacheBtn",
				text : '重置读卡器缓存',
				iconCls : 'icon-redo',
				handler : function() {
					clearMobileCardReaderCache();
				}
			},'-',
			{
				id : "exportBtn",
	...
	function add() {
		...
		window.parent.$("#modalWindow").html("<div id=\"mobileCardReaderModalWindow\" modal=\"true\" shadow=\"false\" minimizable=\"false\" cache=\"false\" maximizable=\"false\" collapsible=\"false\" resizable=\"false\" style=\"margin: 0px;padding: 0px;overflow: auto;\"></div>");
		
		/* 初始化弹出页面的层 */
		window.parent.$("#mobileCardReaderModalWindow").window({
			title : '新增',
			href : "${ctx}/pages/mobile/mobilecardreader_addorupdate.jsp",
			width : 300,
			height : 220,
			onClose: function () {
				window.parent.$("#mobileCardReaderModalWindow").window('destroy');
			},
			onLoad : function() {
	...
	function edit(rowIndex) {
		...
		window.parent.$("#mobileCardReaderModalWindow").window({
			...
			onLoad : function() {
				$.ajax({
					type : "post",
					url : "${ctx}/mobileCardReader.do?method=get",
					data : {
						"id" : rows.id
					},
					dataType : "json",
	...
	function addOrUpdate() {
		var url = "";
		if (operate == "add") {
			url = "${ctx}/mobileCardReader.do?method=add";
		} else if (operate == "update") {
			url = "${ctx}/mobileCardReader.do?method=update";
		}
	...
	function clearMobileCardReaderCache(){
		$.ajax({
			type:"POST",
	        url:"${ctx}/mobileCardReader.do?method=clearmobilecardreadercache",
	        async: false,
	        success: function(data){
	        	if(data.message == "200"){
	        		actionSuccess();
	...
	function remove(){
		...
		window.parent.$.messager.confirm("确认", "是否确认删除", function(r) {
			if(r){
				$.post("${ctx}/mobileCardReader.do?method=delete", {ids:ids}, function(data) {
					if (data.message == "200") {
						actionSuccess();
						$('#tableData').datagrid("reload");
						rows.length=0;
	...
	function exprot() {
		...
		$.ajax({  
	        type:"POST",
	        dataType: "text",
	        url:"${ctx}/mobileCardReader.do?method=getfilepath",
	        data:"equipmentNo="+equipmentNo+"&description="+description+"",
	        async: false,  
	        success: function(data) {
	        	if(data != null && data != ""){
