...
<script type="text/javascript">
	$(function(){
	    ...
	    $("#processStatus").combobox({
		    url:"${ctx}/code.do?method=getcodecache&category=MOBILEMESSAGEPROCESSSTATUS",
		    valueField:'id',
		    textField:'text',
			required:true,
			editable:false
...
<div class="easyui-layout" data-options="fit:true,border:false">
	<div data-options="region:'center',border:false" style="padding: 5px;">
		...
		<form id="tableForm" name="tableForm">
			...
			<input type="hidden" id="targetUris" name="targetUris" />
			<div class="custtop">
				<ul>
					<%if(op.equals("add")){ %>
					<li>
					...
					<li>
						...
						<span class="inputlable">
							<input id="content" name="content" class="easyui-validatebox" data-options="required:true" />
						</span>
					</li>
					
					<li>
						<span class="textlable">处理状态：</span>
						<span class="inputlable">
							<input id="processStatus" name="processStatus" data-options="required:true" />
						</span>
					</li>
...
<script type="text/javascript">
	$(function() {
		...
		$("#tableForm").form({
			...
			onSubmit : function() {
				...
				window.parent.$("#targetUris").val(targetUris);
			
				var flag = $(this).form('validate');
				if (flag) {
                	var targetUris = window.parent.$("#targetUris").val();
                	if (targetUris != null && targetUris != ""){
                		flag = true;
                	}else{
                		window.parent.$.messager.alert("错误信息","请选择用户",'error');
						flag = false;
                	}
				}
				return flag;
	...
	function cancel() {
		$("#mobileMessageModalWindow").window("destroy");
	}
</script>
