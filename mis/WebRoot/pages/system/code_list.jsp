...
<script type="text/javascript">
	...
	function add() {
		...
		window.parent.$("#modalWindow").html("<div id=\"codeModalWindow\" modal=\"true\" shadow=\"false\" minimizable=\"false\" cache=\"false\" maximizable=\"false\" collapsible=\"false\" resizable=\"false\" style=\"margin: 0px;padding: 0px;overflow: auto;\"></div>");
		
		/* 初始化弹出页面的层 */
		window.parent.$("#codeModalWindow").window({
			title : '新增',
			href : "${ctx}/pages/system/code_addorupdate.jsp?op=" + operate,
			width : 300,
			height : 200,
			onClose: function () {
				window.parent.$("#codeModalWindow").window('destroy');
			},
			onLoad : function() {
	...
	function addOrUpdate() {
		var url = "";
		if (operate == "add") {
			url = "${ctx}/code.do?method=add";
		} else if (operate == "update") {
			url = "${ctx}/code.do?method=update";
		}
	...
	function clearCodeCache(){
		$.ajax({
			type:"POST",
	        url:"${ctx}/code.do?method=clearcodecache",
	        async: false,
	        success: function(data){
	        	if(data.message == "200"){
	        		actionSuccess();
	...
	function exprot() {
		...
		$.ajax({  
	        type:"POST",
	        dataType: "text",
	        url:"${ctx}/code.do?method=getfilepath",
