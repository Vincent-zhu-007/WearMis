...
<script type="text/javascript">
	...
	function tableDataInit() {
		$("#tableData").datagrid({
			...
			fit : true,
			url : "${ctx}/province.do?method=list",
	...
	function add() {
		...
		window.parent.$("#modalWindow").html("<div id=\"provinceModalWindow\" modal=\"true\" shadow=\"false\" minimizable=\"false\" cache=\"false\" maximizable=\"false\" collapsible=\"false\" resizable=\"false\" style=\"margin: 0px;padding: 0px;overflow: auto;\"></div>");
		
		/* 初始化弹出页面的层 */
		window.parent.$("#provinceModalWindow").window({
			title : '新增',
			href : "${ctx}/pages/system/province_addorupdate.jsp?op=" + operate,
			width : 300,
			height : 200,
			onClose: function () {
				window.parent.$("#provinceModalWindow").window('destroy');
			},
			onLoad : function() {
	...
	function edit(rowIndex) {
		...
		window.parent.$("#provinceModalWindow").window({
			...
			onLoad : function() {
				$.ajax({
					type : "post",
					url : "${ctx}/province.do?method=get",
					data : {
						"id" : rows.id
					},
					dataType : "json",
	...
	function addOrUpdate() {
		var url = "";
		if (operate == "add") {
			url = "${ctx}/province.do?method=add";
		} else if (operate == "update") {
			url = "${ctx}/province.do?method=update";
		}
	...
	function remove(){
		...
		window.parent.$.messager.confirm("确认", "是否确认删除", function(r) {
			if(r){
				$.post("${ctx}/province.do?method=delete", {ids:ids}, function(data) {
					if (data.message == "200") {
						actionSuccess();
						$('#tableData').datagrid("reload");
						rows.length=0;
	...
	function clearProvinceCache(){
		$.ajax({
			type:"POST",
	        url:"${ctx}/province.do?method=clearprovincecache",
	        async: false,
	        success: function(data){
	        	if(data.message == "200"){
	        		actionSuccess();
	...
	function exprot() {
		...
		$.ajax({  
	        type:"POST",
	        dataType: "text",
	        url:"${ctx}/province.do?method=getfilepath",
