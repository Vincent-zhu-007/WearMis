...
<script type="text/javascript">
	...
	function add() {
		...
		window.parent.$("#modalWindow").html("<div id=\"userModalWindow\" modal=\"true\" shadow=\"false\" minimizable=\"false\" cache=\"false\" maximizable=\"false\" collapsible=\"false\" resizable=\"false\" style=\"margin: 0px;padding: 0px;overflow: auto;\"></div>");
		
		/* 初始化弹出页面的层 */
		window.parent.$("#userModalWindow").window({
			title : '新增',
			href : "${ctx}/pages/system/user_addorupdate.jsp?op=" + operate,
			width : 520,
			height : 400,
			onClose: function () {
				window.parent.$("#userModalWindow").window('destroy');
			},
			onLoad : function() {
				initListboxRemove();
			}
				...
				$.ajax({
					...
					success : function(data) {
						window.parent.$('#tableForm').form('load', {
							id:data.id,
							userInRoleId:data.userInRoleId,
	...
	function addOrUpdate() {
		var url = "";
		if (operate == "add") {
			url = "${ctx}/user.do?method=add";
		} else if (operate == "update") {
			url = "${ctx}/user.do?method=update";
		}
	...
	function initListboxRemove(){
		$.ajax({
			url:"${ctx}/code.do?method=getcodehtmloptionsbycategory&category=ROLE",
			type:"POST",
			async:false,
			dataType:"text",
			success:function(data){
	...
	function initListboxRemoveAndListboxAdd(id){
		$.ajax({
			url:"${ctx}/code.do?method=getremoveandaddcodehtmloptionsbycategory&category=ROLE&id=" + id,
			type:"POST",
			async:false,
			dataType:"text",
			success:function(data){
	...
	function remove(){
		...
		window.parent.$.messager.confirm("确认", "是否确认删除", function(r) {
			if(r){
				$.post("${ctx}/user.do?method=delete", {ids:ids}, function(data) {
					if (data.message == "200") {
						actionSuccess();
						$('#tableData').datagrid("reload");
						rows.length=0;
	...
	function exprot() {
		...
		$.ajax({  
	        type:"POST",
	        dataType: "text",
	        url:"${ctx}/user.do?method=getfilepath",
