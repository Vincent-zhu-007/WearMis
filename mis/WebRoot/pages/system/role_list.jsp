...
<script type="text/javascript">
	...
	function add() {
		...
		window.parent.$("#modalWindow").html("<div id=\"roleModalWindow\" modal=\"true\" shadow=\"false\" minimizable=\"false\" cache=\"false\" maximizable=\"false\" collapsible=\"false\" resizable=\"false\" style=\"margin: 0px;padding: 0px;overflow: auto;\"></div>");
		
		/* 初始化弹出页面的层 */
		window.parent.$("#roleModalWindow").window({
			title : '新增',
			href : "${ctx}/pages/system/role_addorupdate.jsp?op=" + operate,
			width : 520,
			height : 500,
			onClose: function () {
				window.parent.$("#roleModalWindow").window('destroy');
			},
			onLoad : function() {
				//初始化树
	        	window.parent.$("#roleMenu").tree({
	        		url: "${ctx}/role.do?method=initmenu",
	        		checkbox: true,
			...
			onLoad : function() {
				$.ajax({
					...
					success : function(data) {
						...
						window.parent.$("#roleMenu").tree({
			        		url: "${ctx}/role.do?method=initmenubypermission&roleId=" + rows.id,
			        		checkbox: true,
			        		animate:false,/* 是否有动画效果 */
			        		onClick:function(node){
			        			window.parent.$(this).tree('toggle', node.target);
	...
	function addOrUpdate() {
		var url = "";
		if (operate == "add") {
			url = "${ctx}/role.do?method=add";
		} else if (operate == "update") {
			url = "${ctx}/role.do?method=update";
		}
	...
	function parentClearMenuConfigCache(){
		$.ajax({
	        ...
	        success: function(data){
	        	if(data.message == "200"){
	        		window.parent.$("#roleMenu").tree('reload');
	        		actionSuccess();
	        	}else{
	        		actionFailure();
	        	}
	...
	function remove(){
		...
		window.parent.$.messager.confirm("确认", "是否确认删除", function(r) {
			if(r){
				$.post("${ctx}/role.do?method=delete", {ids:ids}, function(data) {
					if (data.message == "200") {
						actionSuccess();
						$('#tableData').datagrid("reload");
						rows.length=0;
	...
	function exprot() {
		...
		$.ajax({  
	        type:"POST",
	        dataType: "text",
	        url:"${ctx}/role.do?method=getfilepath",
