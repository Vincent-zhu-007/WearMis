...
<script type="text/javascript">
	...
	function tableDataInit() {
		$("#tableData").datagrid({
			...
			fit : true,
			url : "${ctx}/weiXinRole.do?method=list",
	...
	function add() {
		...
		window.parent.$("#modalWindow").html("<div id=\"weiXinRoleModalWindow\" modal=\"true\" shadow=\"false\" minimizable=\"false\" cache=\"false\" maximizable=\"false\" collapsible=\"false\" resizable=\"false\" style=\"margin: 0px;padding: 0px;overflow: auto;\"></div>");
		
		/* 初始化弹出页面的层 */
		window.parent.$("#weiXinRoleModalWindow").window({
			title : '新增',
			href : "${ctx}/pages/weixin/weixinrole_addorupdate.jsp?op=" + operate,
			width : 515,
			height : 500,
			onClose: function () {
				window.parent.$("#weiXinRoleModalWindow").window('destroy');
			},
			onLoad : function() {
				//初始化树
	        	window.parent.$("#weiXinRoleMenu").tree({
	        		url: "${ctx}/weiXinRole.do?method=initweixinmenu",
	        		checkbox: true,
	...
	function edit(rowIndex) {
		...
		window.parent.$("#weiXinRoleModalWindow").window({
			...
			onLoad : function() {
				$.ajax({
					type : "post",
					url : "${ctx}/weiXinRole.do?method=get",
					data : {
						"id" : rows.id
					},
					dataType : "json",
					...
					success : function(data) {
						...
						window.parent.$("#weiXinRoleMenu").tree({
			        		url: "${ctx}/weiXinRole.do?method=initweixinmenubypermission&roleId=" + rows.id,
			        		checkbox: true,
			        		animate:false,/* 是否有动画效果 */
			        		onClick:function(node){
			        			window.parent.$(this).tree('toggle', node.target);
	...
	function addOrUpdate() {
		var url = "";
		if (operate == "add") {
			url = "${ctx}/weiXinRole.do?method=add";
		} else if (operate == "update") {
			url = "${ctx}/weiXinRole.do?method=update";
		}
	...
	function parentClearWeiXinChannelCache(){
		$.ajax({
			type:"POST",
	        ...
	        success: function(data){
	        	if(data.message == "200"){
	        		window.parent.$("#weiXinRoleMenu").tree('reload');
	        		actionSuccess();
	        	}else{
	        		actionFailure();
	        	}
	...
	function remove(){
		...
		window.parent.$.messager.confirm("确认", "是否确认删除", function(r) {
			if(r){
				$.post("${ctx}/weiXinRole.do?method=delete", {ids:ids}, function(data) {
					if (data.message == "200") {
						actionSuccess();
						$('#tableData').datagrid("reload");
						rows.length=0;
	...
	function exprot() {
		...
		$.ajax({  
	        type:"POST",
	        dataType: "text",
	        url:"${ctx}/weiXinRole.do?method=getfilepath",
