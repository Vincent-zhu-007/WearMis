		...
		<form id="tableForm" name="tableForm">
			...
			<div class="custbottom">
				<span class="custbottom_title">
					权限列表
					<a href="javascript:;" onclick="clearWeiXinChannelCache()">重置微信权限缓存</a>
				</span>
				<ul id="weiXinRoleMenu"></ul>
			</div>
		</form>
	</div>
...
<script type="text/javascript">
	$(function() {
		...
		$("#tableForm").form({
			url : "",
			onSubmit : function() {
				var nodes = window.parent.$("#weiXinRoleMenu").tree('getChecked');
				var permission = "";
				
				if(nodes != null && nodes != ""){
					$.each(nodes, function(i, node) {
						if(nodes.length - 1 == i){
							permission += node.attributes.channelCodeName;						
						}else{
							permission += node.attributes.channelCodeName + ",";
						}
					});
	...
	function clearWeiXinChannelCache(){
		...
		var frameId = "iframeId_" + tabId;
		var currentFrames = window.frames[frameId];
		currentFrames.parentClearWeiXinChannelCache();
	}
	...
	function cancel() {
		$("#weiXinRoleModalWindow").window("destroy");
	}
</script>
