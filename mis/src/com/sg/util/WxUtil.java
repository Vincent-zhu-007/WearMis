...
public class WxUtil {
	...
	public static String processWxRequestByPostCreateMenu(String accessToken, String jsonContent){
		...
		try {
			String data = sendPostJson(url, jsonContent);
			
			JSONObject jsonObjectData = JSONObject.fromObject(data);
			
			String errCode = jsonObjectData.get("errcode").toString();
			
			if(errCode != null && !errCode.equals("") && errCode.equals("0")){
				jsonObjectResult.put("result", "1");
				jsonObjectResult.put("data", "200");
			}else {
				String errMsg = jsonObjectData.get("errmsg").toString();
				
				jsonObjectResult.put("result", "0");
				jsonObjectResult.put("message", errMsg);
			}
		} catch (Exception e) {
			// TODO Auto-generated catch block
			jsonObjectResult.put("result", "0");
