...
public class WeiXinChannelService implements BaseEntityManager<WeiXinChannel> {
	...
	public String getWeiXinMenuJson() {
		...
		JSONObject jsonObjectWeiXinMenu = buildWeiXinMenuTree(level2WeiXinChannels);
		
		return jsonObjectWeiXinMenu.toString();
	}
	
	public JSONObject buildWeiXinMenuTree(List<WeiXinChannel> level2WeiXinChannels) {
		...
		if (level2WeiXinChannels != null && level2WeiXinChannels.size() > 0) {
			for (int i = 0; i < level2WeiXinChannels.size(); i++) {
				...
				String channelCodeName = level2WeiXinChannel.getChannelCodeName();
				...
				for (WeiXinChannel buttonWeiXinChannel : weiXinChannels) {
					if(buttonWeiXinChannel.getChannelLevelCode().equals("Button") && buttonWeiXinChannel.getChannelParentCode().equals(channelCodeName)) {
						buttonWeiXinChannels.add(buttonWeiXinChannel);
					}
				}
				
				if(buttonWeiXinChannels != null && buttonWeiXinChannels.size() > 0){
					JSONArray jsonArrayButton = new JSONArray();
					
					for (WeiXinChannel buttonWeiXinChannel : buttonWeiXinChannels) {
						JSONObject jsonObjectButton = new JSONObject();
						
						String type3 = buttonWeiXinChannel.getType();
						String name3 = buttonWeiXinChannel.getDescription();
						String key3= buttonWeiXinChannel.getTypeValue() == null ? "" : buttonWeiXinChannel.getTypeValue();
						
						jsonObjectButton.put("name", name3);
						jsonObjectButton.put("type", type3);
						
						if(type3.equals("click") || type3.equals("scancode_push")){
							jsonObjectButton.put("key", key3);
						}else if (type3.equals("view")) {
							jsonObjectButton.put("url", key3);
						}
						
						jsonArrayButton.add(jsonObjectButton);
					}
					
					jsonObject1.put("sub_button", jsonArrayButton.toString());
				}else {
		...
		if(jSONArray != null && jSONArray.size() > 0){
			jsonObjectResult.put("button", jSONArray.toString());
		}
